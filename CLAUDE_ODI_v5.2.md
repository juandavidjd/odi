...
# Incluye: Dualidad de Voz + Observabilidad + Shopify Connector + CES
...
16. [Conectores (v5.2)](#16-conectores-v52)
...
### 16.1 Arquitectura de Conectores

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  CONECTOR = Acción + CES + Auditoría                       │
│                                                             │
│  ┌─────────────────┐                                        │
│  │ @RequireHuman   │  ← Decorador CES                       │
│  │  Confirmation   │                                        │
│  └────────┬────────┘                                        │
│           │                                                 │
│           ▼                                                 │
│  ┌─────────────────┐    ┌─────────────────┐                │
│  │ CESEvaluator    │───▶│ PolicyDecision  │                │
│  │ (umbral/riesgo) │    │ (allow/confirm) │                │
│  └─────────────────┘    └─────────────────┘                │
│           │                                                 │
│           ▼                                                 │
│  ┌─────────────────┐                                        │
│  │ Audit Ledger    │  ← Registro inmutable                  │
│  └─────────────────┘                                        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 16.2 Shopify Connector

**Tiendas configuradas:** 10

| Store ID | Nombre | Productos |
|----------|--------|-----------|
| bara | Bara Importaciones | ~700 |
| yokomar | Yokomar Repuestos | ~650 |
| kaiqi | Kaiqi Motos | ~800 |
| dfg | DFG Parts | ~550 |
| duna | Duna Accesorios | ~450 |
| imbra | Imbra Motos | ~600 |
| japan | Japan Repuestos | ~720 |
| leo | Leo Motopartes | ~580 |
| store | SRM Store | ~1200 |
| vaisand | Vaisand Motos | ~490 |

### 16.3 Operaciones

**Read-only (sin CES):**
- `get_products(store_id, limit)`
- `get_product(store_id, product_id)`
- `get_inventory(store_id)`
- `get_orders(store_id)`

**Write (con CES):**
- `update_price(store_id, product_id, new_price)` → Requiere confirmación si cambio > 10%
- `update_inventory(store_id, item_id, quantity)` → Requiere confirmación si > 50 items
- `create_order(store_id, customer, items)` → SIEMPRE requiere confirmación
- `create_product(store_id, data)` → Valida margen mayorista

### 16.4 Decorador @RequireHumanConfirmation

```python
@RequireHumanConfirmation(
    action_type=ActionType.SHOPIFY_CREATE_ORDER,
    cost_estimator=lambda **kw: kw.get("total_price", 0),
    risk_level=RiskLevel.HIGH,
    evidence_builder=lambda r: r.get("admin_url")
)
async def create_order(self, store_id, customer, items, **kwargs):
    ...
```
...
| Umbral | Valor | Acción |
|--------|-------|--------|
| `order_auto_approve` | $200,000 COP | CREATE_ORDER |
| `payment_auto_approve` | $100,000 COP | WOMPI |
| `price_change_auto_approve` | 10% | UPDATE_PRICE |
| `inventory_bulk_threshold` | 50 items | UPDATE_INVENTORY |

### 16.6 Filtro de Precios Mayorista

**Margen mínimo:** 15%

```python
# Precio inválido → PriceValidationError
validator.validate(52000, 50000)  # ✗ Solo 4% margen

# Precio válido
validator.validate(60000, 50000)  # ✓ 20% margen
```
...
```bash
# .env - Tokens Shopify
SHOPIFY_BARA_TOKEN=shpat_xxxxx
SHOPIFY_YOKOMAR_TOKEN=shpat_xxxxx
# ... (10 tokens)
```
...
| **v5.2** | **2026-01-24** | **Shopify Connector + CES + Filtro Mayorista** | |
